...
| `/api/findings` + `?afterKey=` cursor | ✅ **Correct bulk-export path.** Verified live crossing 12,000 records (CUSTOMER_C): 0 dups, 0 errors. Keyset cursor has **no 10K offset limit** — offset-based limits don't apply. |
| CVE filter key | ✅ `cve` (list). ❌ `cveId` is **silently ignored** and returns the entire unfiltered tenant (matched 8.67M on CUSTOMER_C). Use `cve` on both endpoints. |
...
### `/api/findings` filters (verified live on CUSTOMER_C)
