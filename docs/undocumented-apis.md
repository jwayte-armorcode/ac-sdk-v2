...
|------|-----------|--------|
| [Ticketing — Azure Boards mappings](#ticketing--azure-boards-mappings) | `/user/tickets/jira/*` CRUD | Verified from UI (CUSTOMER_B, 2026-07) |
...
- **Path values are single backslashes on the wire.** Area/Iteration values are
  ADO tree paths like `\CUSTOMER_B.Work\Operations\Security Team` — one backslash
...
Verified from live web-app network captures on the **CUSTOMER_B** tenant
...
| Create (project `CUSTOMER_B.Work`) | `POST /user/tickets/jira/configuration` | ✅ mapping created — confirmed dual-representation + project-specific field keys |
