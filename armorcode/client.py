...
class ArmorCodeClient:
    ...
    def create_azure_board_config(
        ...
        **all** connections) and, if any requested repo is already mapped,
        ...
        Args:
            project_key: Azure DevOps project key (e.g. ``"CUSTOMER_B.Work"``).
    ...
    def get_users_flat(self):
        ...
        name/email lookups. Note many CUSTOMER_A accounts have no display name, so
    ...
    def get_roles(self):
        ...
        * **teamInfo roles** — per-team membership roles (e.g. ``"CUSTOMER_A
          Executive"``, ``"CUSTOMER_A PM"``) used in a user's ``teamInfo`` and in
        ...
        * **tenantRoles** — account-level roles accepted by :meth:`create_user`.
          ...
          CUSTOMER_A Security Engineer, CUSTOMER_A Engineering Manager, CUSTOMER_A
          Security Ambassadors, CUSTOMER_A PM. NOT valid as tenantRoles: "CUSTOMER_A
          Executive", "CUSTOMER_A IT Manager", "CUSTOMER_A Software Engineer",
          "CUSTOMER_A IT Engineer".
    ...
    def create_team_member_user(self, email, *, disable_login=False,
        ...
        * It accepts NO name field, so the display name will be the email (an
          "email-only" account, like many CUSTOMER_A team-owner users).
    ...
    def add_user_to_team(self, user, team_id, team_name, role_name, role_id,
        ...
        Args:
            ...
            role_name: teamInfo role NAME (e.g. ``"CUSTOMER_A Executive"``).
    ...
    # The 5 team owner-field keys (UI labels in parens are the CUSTOMER_A tenant's
    # global-settings Titles): complianceOwner (AppSec Engineer), securityOwner
    # (Security Ambassador), engineeringOwner (CUSTOMER_A Director), businessOwner
    # (CUSTOMER_A PM), supportOwner (CUSTOMER_A VP).
    _TEAM_OWNER_KEYS = (
